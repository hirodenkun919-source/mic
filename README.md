...
Web Speech APIを使用した音声入力ツール。Safari のブックマークレットで ChatGPT、Gemini、Claude など、**どのWebページでも使える** シンプルなツールです。

---

## 📱 インストール方法（3ステップ）

### **Step 1️⃣ ブックマークレットコードをコピー**
...
### **Step 2️⃣ Safari でブックマークを作成**

1. Safari を開く
2. 下部の **「⬆️ シェア」** をタップ
3. 「ブックマーク」をタップ
4. 名前を「🎤 音声入力」に変更
5. 保存場所を「**ブックマークバー**」に変更
6. 「保存」をタップ

---

### **Step 3️⃣ ブックマークを編集して貼り付け**

1. Safari のブックマークバーで「🎤 音声入力」を長押し
2. 「編集」をタップ
3. URL欄をタップしてすべて削除
4. **上でコピーしたコード全体を貼り付ける**
5. 「完了」をタップ
...
1. **ブックマークをタップ** → 画面右下に 🎤 オレンジボタンが表示
2. **ボタンをタップ** → 「音声認識中...」に変わります
3. **日本語で話す** → 自動的にテキストに変換
4. **テキストが自動挿入** → ChatGPT、Gemini、Claude など、開いているアプリのテキストボックスに挿入
5. **×ボタン** → タップして終了
...
### ❌ 「Speech非対応」と表示される
→ Safari が Web Speech API に対応していないバージョンです。iOS 14.5以降にアップデートしてください。

### ❌ マイクへのアクセスを求められない / 音声が認識されない
→ 設定 > プライバシー > マイク で、Safari に **マイクへのアクセスを許可** してください。

### ❌ テキストが入力されない
→ テキストボックスにカーソルがある状態（フォーカス）にしてから、ボタンをタップしてください。

### ❌ ボタンがどこにも表示されない
1. Safari を再読み込みしてください
2. ブックマークのURLが正しく貼り付けられているか確認してください
3. ブックマークの編集画面を開き、URLが「`javascript:`」で始まっているか確認
...
- **GitHub**: https://github.com/hirodenkun919-source/mic
