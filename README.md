...
#### Step 2️⃣ ブックマークを作成して貼り付ける

**Safari での登録方法：**

1. Safari を開く
2. 下部の **「⬆️ シェア」** → 「ブックマーク」をタップ
3. 名前を「🎤 音声入力」に変更
4. 保存場所を「ブックマークバー」に変更して保存
5. ブックマークバーの新しいブックマークを長押し → 「編集」
6. URLをすべて削除して、**コピーしたコード全体を貼り付ける**
7. 「完了」をタップ

**Chrome での登録方法：**

1. Chrome を開く
2. 右上の **「︙」メニュー** → 「ブックマーク」 → 「ブックマークを追加」
3. 名前を「🎤 音声入力」に変更
4. URLをすべて削除して、**コピーしたコード全体を貼り付ける**
5. 「保存」をクリック

---

### **方法②：ドラッグ&ドロップで登録（Safari のみ）**
...
| サービス | 対応状況 | 備考 |
|---------|--------|------|
| **ChatGPT** | ✅ 確認済み | Webアプリで動作確認 |
| **Gmail** | ⚠️ 対応予定 | Textareaセレクタで理論上対応（要検証） |
| **Safari** | ✅ iOS 14.5以降で動作確認 | 推奨ブラウザ |
| **Chrome** | ⚠️ 対応予定 | Web Speech API対応（要検証） |
| **メモアプリ** | ✅ 対応 | Textareaで動作 |
| **Google Docs** | ✅ contenteditable対応 | 動作確認済み |

**⚠️ Gmail・Chrome の詳細検証中です。問題があればIssueを報告してください。**
...
- **Safari**: 設定 > プライバシー > マイク > このサイト（またはこのアプリ）を許可
- **Chrome**: サイト設定 > マイク > 許可
...
### ❌ 音声が認識されない
1. マイクが正常に動作しているか確認してください
2. 周囲がうるさい場所ではないか確認してください
3. ブラウザを再読み込みして再度試してください
...
- **言語**: 日本語 (ja-JP)
- **オフライン対応**: 一部オフラインで動作（デバイス側の音声エンジンを使用）
- **権限**: マイクへのアクセス許可が必要
- **ブラウザ互換性**: Chrome 25+, Edge 79+, Safari 14.5+, Firefox 無効
...
- **公開ページ**: https://hirodenkun919-source.github.io/mic/

---

## 💬 問題報告

Gmail、Chrome などで問題が発生した場合は、GitHubの Issueで報告してください！
